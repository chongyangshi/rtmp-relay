...
sed -i "s|INGRESS_SUBNETS|$INGRESS_SUBNETS|g" /etc/nginx/nginx.conf
...
sed -i "s|INGRESS_STREAM_KEY|$INGRESS_STREAM_KEY|g" /etc/nginx/nginx.conf

# PUSH_RTMP_URL defines where should the inbound stream be relate to a third-party ingestion
# endpoint, such as Twitch.
sed -i "s|PUSH_RTMP_URL|$PUSH_RTMP_URL|g" /etc/nginx/nginx.conf
